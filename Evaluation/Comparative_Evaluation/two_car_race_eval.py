...
Usage:  python.bat two_car_race_eval.py <original|stadium> <n_starts> <control_steps> <label> [label ...]
        (label = checkpoint_backups/car_race_ppo_model_<label>.zip)
...
sys.path.insert(0, f"{THESIS}/Racing/Two_Car_Self_Play_Racing")
...
if TRACK == "original":
    ...
    STAGE = f"{THESIS}/Racing/Two_Car_Self_Play_Racing/race_deploy.usd"
else:
    from car_unknown_track_env import CHASSIS_Z, OFF_TRACK_MARGIN, build_centerline, wrap_to_pi  # noqa: E402
    STAGE = f"{THESIS}/Racing/Two_Car_Self_Play_Racing/unknown_track.usd"
...
for label in LABELS:
    policy = PPO.load(f"{THESIS}/checkpoint_backups/car_race_ppo_model_{label}.zip", device="cpu")
    if TRACE:
