"""Racing-line variant of car_track_vec_env.py: identical architecture,
same stage (vec_train_car2.usd), same 7-dim curvature-aware observation -
only the reward weights differ.

The centerline-following reward (car_track_vec_env.py) penalizes lateral
offset heavily (-0.6/m) to keep the car hugging the center. But the track's
curbs already physically bound how far the car can stray - nothing stops
it from using the full track width to carry more speed through a corner
except that lateral penalty. This variant loosens it substantially and
rewards throttle more, so a fast line (cutting toward the inside of a
corner, using the outside on the approach) has room to actually pay off
instead of being reward-dominated by staying centered.

Kept as a fully separate lineage/checkpoint from the centerline-following
policy, for a direct comparison: does letting the reward permit racing-line
behavior actually produce faster lap times, or does it just make the
policy less stable without changing what it learns to do?
"""
...
STAGE_PATH = "C:/Users/sanja/Desktop/thesis/Vec_Curvature_Lineage/vec_train_car2.usd"
NUM_ENVS = 10
GRID_COLS = 3
GRID_SPACING = 60.0
...
# Racing-line reward: loosened from the centerline-following variant's 0.6.
# Curbs (physical, collision=True) still hard-bound the track - this just
# stops the reward itself from forcing the car back to center when using
# more of the track width would let it carry more speed through a corner.
LATERAL_PENALTY_WEIGHT = 0.1
OFF_TRACK_PENALTY = 30.0
THROTTLE_BONUS_WEIGHT = 0.4
LOOKAHEAD_DIST = 5.0
...
class CarTrackRacinglineVecEnv(VecEnv):
