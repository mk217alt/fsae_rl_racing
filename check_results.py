...
# Run from the repository root:   python check_results.py
import csv, glob, json, os, statistics, zipfile

EV = "Evaluation/Comparative_Evaluation/"
...
for p in ["Baselines/Single_Car_Baseline/car2_ppo_model.zip",
          "Racing/Vec_Curvature_Lineage/car2_vec_ppo_model.zip",
          "Racing/Racing_Line_Variant/car2_racingline_ppo_model.zip",
          "Racing/Two_Car_Self_Play_Racing/car_race_ppo_model.zip"]:
...
try:
    import numpy as np
    for f in sorted(glob.glob("Baselines/BC_Finetuning_Experiment/human_driving_data*.npz")):
