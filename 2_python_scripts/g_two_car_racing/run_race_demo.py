...
def _telemetry_writer():
    while True:
        ...
        with open(f"{ROOT}/run_output/race_demo_telemetry.txt", "a") as f:
            f.write(f"t={t} carA={pa0} carB={pb0}\n")

        total = sum(recovery_snapshot.values())
        with open(f"{ROOT}/run_output/race_demo_recovery_stats.txt", "w") as f:
...
with open(f"{ROOT}/run_output/run_race_demo_ready.txt", "w") as f:
    f.write("ready\n")
