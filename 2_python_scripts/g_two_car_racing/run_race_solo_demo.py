...
def telemetry_step(step_size):
    ...
    with open(f"{ROOT}/run_output/race_solo_demo_telemetry.txt", "a") as f:
        f.write(f"t={_telemetry_counter[0]} carA={pa[0]}\n")

    total = sum(recovery_counts.values())
    with open(f"{ROOT}/run_output/race_solo_demo_recovery_stats.txt", "w") as f:
...
with open(f"{ROOT}/run_output/run_race_solo_demo_ready.txt", "w") as f:
    f.write("ready\n")
