...
OUT_PATH = f"{ROOT}/5_data/human_driving_data.npz"
...
def drive_step(step_size):
    ...
    if _step_counter[0] % ACTION_REPEAT == 0:
        ...
        if len(recorded_obs) % 500 == 0:
            save()
            with open(f"{ROOT}/run_output/record_driving_progress.txt", "w") as f:
...
with open(f"{ROOT}/run_output/record_driving_ready.txt", "w") as f:
    f.write("ready\n")
...
save()
with open(f"{ROOT}/run_output/record_driving_done.txt", "w") as f:
