"""Builds a single-pair stage on a track layout the two-car self-play policy
has NEVER seen during training, for a zero-shot generalization test.

Every training round (run1-49+) used the exact same symmetric rounded-square
track: HALF_STRAIGHT=8.0, CORNER_RADIUS=10.0 (car_race_vec_env.py). This
stage instead builds an elongated, asymmetric "stadium" layout - much longer
horizontal straights (long enough to test sustained high-speed driving well
past anything trained), much shorter vertical straights, and noticeably
tighter corners than any curvature the policy has trained against:
  HALF_LENGTH_X=20.0 (horizontal straights: 40m, vs trained 16m - 2.5x longer)
  HALF_LENGTH_Y=4.0  (vertical straights: 8m, vs trained 16m)
  CORNER_RADIUS=6.0  (vs trained 10.0 - tighter turns)
Track width (9.0m) is kept identical so friction/dynamics stay comparable -
this isolates the track-GEOMETRY generalization question specifically.

Otherwise this is build_race_deploy_stage.py's exact construction (analytic
ground plane, solver iteration counts, filtered chassis-vs-wheel pairs, gap
fillers, checkered start line) - all the physics-stability fixes that
lineage already earned stay in place here.
"""
...
OUT_PATH = "C:/Users/sanja/Desktop/thesis/Racing/Two_Car_Self_Play_Racing/unknown_track.usd"
...
def build_track(track_path, offset):
    ...
    points = list(reversed(points))  # clockwise, matching car_race_vec_env's convention
...
with open("C:/Users/sanja/Desktop/thesis/build_unknown_track_deploy_done.txt", "w") as f:
    f.write(f"built unknown-track deploy stage at {OUT_PATH}\n")
